...
res_real <- res_perm[1,]; res_real[,3:8] <- NA

percentile=0.01
res_real$fc_L13 <- sum(info$L13.pval.ML <= percentile)/(length(info$L13.pval.ML)*percentile)
res_real$fc_D13 <- sum(info$D13.pval.ML <= percentile)/(length(info$D13.pval.ML)*percentile)
res_real$p_L13 <- binom.test(sum(info$L13.pval.ML <= percentile), n = length(info$L13.pval.ML), p=percentile)$p.value
res_real$p_D13 <- binom.test(sum(info$D13.pval.ML <= percentile), n = length(info$D13.pval.ML), p=percentile)$p.value
rm(percentile, info)
...
load("../DATA/pvalues.pseudohaploid.n_neutral_250.RData")
rm(calc_pval_D13, calc_pval_L12, calc_pval_L13, site, design, design2, input_fst, input_maf, drop_samples_missing, n_less, info_neut)
info <- info[!info$type == "neut",]
res_pseudohaploid <- res_real[1,]; res_pseudohaploid[,3:8] <- NA

info <- info[rowSums(is.na(info)) == 0,]

percentile=0.01
res_pseudohaploid$fc_L13 <- sum(info$L13.pval <= percentile)/(length(info$L13.pval)*percentile)
res_pseudohaploid$fc_D13 <- sum(info$D13.pval <= percentile)/(length(info$D13.pval)*percentile)
res_pseudohaploid$p_L13 <- binom.test(sum(info$L13.pval <= percentile), n = length(info$L13.pval), p=percentile)$p.value
res_pseudohaploid$p_D13 <- binom.test(sum(info$D13.pval <= percentile), n = length(info$D13.pval), p=percentile)$p.value
rm(percentile, info)
